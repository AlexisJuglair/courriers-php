...
$erreur = (isset($_POST['courriers'][0])) ? $erreur + 0 : $erreur + 4;
if ($erreur == 0) 
{
    $erreur = (count($_POST['courriers']) == 1) ? $erreur + 0 : $erreur + 8;
}

if ($erreur == 0) 
{
    // foreach ($_POST['courriers'] as $id_courrier) 
    // {
        $erreur = 0;
        $erreur = (ctype_digit($_POST['courriers'][0])) ? $erreur + 0 : $erreur + 16;
        $id_courrier = $_POST['courriers'][0];
        //==========================================================================================
        ...
        if ($erreur == 0) {
            ...
            $erreur = (count($courrier) > 0)  ? $erreur + 0 : $erreur + 32;
        }
        ...
        if ($erreur == 0) {
            ...
            $erreur = (count($utilisateur) > 0) ? $erreur + 0 : $erreur + 64;
        }
        ...
        if ($erreur == 0) {
            ...
            $erreur = (count($destinataire) > 0) ? $erreur + 0 : $erreur + 128;
        }
        ...
        if ($erreur == 0) {
            ...
            if ($_GET['cmd'] == 'imprimer') 
            {
                $pdf->Output('courrier.pdf', 'I');
                sleep(10);
            }

            if ($_GET['cmd'] == 'telecharger') 
            {
            ...
            if (($erreur & 8) == 8)
                print("Plusiuers courriers sélectionnés\n");
