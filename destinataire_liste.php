<?php
    ...
    if($errors->check($session->check(),32768))
    {
        ...
        $header->a('','utilisateur_desinscription.php','Se désinscrire',['title'=>"Se désinscrire", "onclick"=>"return confirm('Êtes-vous sûr de vouloir vous désinscrire ?')"]);
            ...
            $header->space();
            $header->a('','liste.php', 'Liste des courriers',['title'=>"Voir tous les courriers."]);
        ...
        $main = new HTML();
            $db = new DB();
            $destinataires = $db->sql("SELECT `id`,`titre`,`prenom`,`nom`,`fonction`,`denomination`,`localite` FROM `destinataires` WHERE utilisateur_id = {$_SESSION["uid"]} AND `status` IS NULL ORDER BY `prenom`, `nom`, `denomination`;");
